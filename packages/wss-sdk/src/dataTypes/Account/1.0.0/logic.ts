...
export function getAccountId(accountMetadata: AccountMetadata) {
  const aggregatedId =
  `${accountMetadata.type === "address"? accountMetadata.address : accountMetadata.xPub}-${accountMetadata.derivationPath}-${accountMetadata.currencyId}-${accountMetadata.derivationMode}`;
